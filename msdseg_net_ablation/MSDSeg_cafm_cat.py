...
class MSDSeg(nn.Module):
    ...
    def __init__(self, num_classes=19,
        ...
        self.seghead = segmenthead(224, 128, num_classes)
    ...
    def forward(self, m):
        ...
        res = []
        res.append(F.interpolate(x16, size=x4.shape[-2:], mode='bilinear', align_corners=False))
        res.append(F.interpolate(x8, size=x4.shape[-2:], mode='bilinear', align_corners=False))
        res.append(x4)
        out = F.interpolate(self.seghead(torch.cat(res, dim=1)), size=m.shape[-2:], mode='bilinear',
                            align_corners=False)
        return out
