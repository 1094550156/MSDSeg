...
class MSDSeg(nn.Module):
    ...
    def forward(self, m):
        ...
        H, W = x16.shape[-2:]
        trans16 = self.trans(nn.functional.adaptive_avg_pool2d(x16, (H // 2, W // 2)))
        fu16 = self.fu16(F.interpolate(trans16, size=x8.shape[-2:], mode='bilinear', align_corners=False), x8)
