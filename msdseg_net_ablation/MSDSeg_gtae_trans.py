...
class MSDSeg(nn.Module):
    ...
    def __init__(self, num_classes=19,
                 drop_path_rate=0.1, depths=2, dilations=[1, 4, 8],
        ...
        self.fu16 = Bag(224, 128, 128)
        self.fu8 = Bag(128, 64, 64)
        self.seghead = segmenthead(64, 128, num_classes)
    ...
    def forward(self, m):
        ...
        res = []
        H, W = x16.shape[-2:]
        res.append(nn.functional.adaptive_avg_pool2d(x16, (H // 2, W // 2)))
        res.append(nn.functional.adaptive_avg_pool2d(x8, (H // 2, W // 2)))
        res.append(nn.functional.adaptive_avg_pool2d(x4, (H // 2, W // 2)))
        trans16 = self.trans(torch.cat(res, dim=1))
        fu16 = self.fu16(F.interpolate(trans16, size=x16.shape[-2:], mode='bilinear', align_corners=False), x16)
        fu8 = self.fu8(F.interpolate(fu16, size=x8.shape[-2:], mode='bilinear', align_corners=False), x8)
        out = F.interpolate(self.seghead(fu8), size=m.shape[-2:], mode='bilinear', align_corners=False)
        return out
