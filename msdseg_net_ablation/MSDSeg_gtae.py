...
class MSDSeg(nn.Module):
    ...
    def __init__(self, num_classes=19,
                 drop_path_rate=0.1, depths=1, dilations=[1, 4, 8],
    ...
    def forward(self, m):
        ...
        fu8 = self.fu8(F.interpolate(trans16, size=x8.shape[-2:], mode='bilinear', align_corners=False), x8)
        fu4 = self.fu4(F.interpolate(fu8, size=x4.shape[-2:], mode='bilinear', align_corners=False), x4)
        out = F.interpolate(self.seghead(fu4), size=m.shape[-2:], mode='bilinear', align_corners=False)
        return out
