...
from competitors_models.stdcnet_seg import BiSeNet
from competitors_models.SSFPN import SSFPN
...
def show_camvid_model(num_images,skip):
    import random
    ...
    model = ZNet_v2(
        num_classes=11,
        pretrained="checkpoints/1000epochs_Zdence_mult_camvid1_run1",
    ...
    config["train_crop_size"]=[720,960]
    config["train_max_size"]=720
    config["train_min_size"]=720
    config["dataset_dir"]="../data/camvid/"
    seed=0
    torch.manual_seed(seed)
    random.seed(seed)
    ...
    colours = get_colors_camvid(train_set.color_to_class)
    _show_mask = lambda image: show_camvid_mask(image, colours)
    show(model,train_loader,device,_show_mask,num_images=num_images,skip=skip,images_per_line=images_per_line)


def show_camvid_bisenet_model(num_images,skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "../configs/camvid_200epochs.yaml"
    with open(config_filename) as file:
        config=yaml.full_load(file)
    from competitors_models.bisenetv1_camvid import BiSeNetV1
    model = BiSeNetV1(
        n_classes=11,
        pretrained="checkpoints/bisenet2_camvid_9.3_run1",
        change_num_classes=True
    )
    num_images=num_images
    images_per_line=1
...
def show_camvid_ssfpn_model(num_images,skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "../configs/camvid_200epochs.yaml"
    with open(config_filename) as file:
    ...
    model = SSFPN(
        'resnet34',
        ResNet34M = True,
        pretrained="checkpoints/ssfpn_camvid_IM_res34M_run1",
        change_num_classes=True,
        classes=11,
    )
    num_images=num_images
...
def show_camvid_stdc_seg_model(num_images,skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "../configs/camvid_200epochs.yaml"
    with open(config_filename) as file:
    ...
    model = BiSeNet(
        'STDCNet1446',
        n_classes=11,
        pretrained="checkpoints/stdcnet_seg_camvid_9.3_run1",
        change_num_classes=True
    )
    num_images=num_images
    images_per_line=1
...
if __name__=="__main__":
    ...
    num_images = 1
    skip = 199
    show_camvid_model(num_images,skip)
    show_camvid_stdc_seg_model(num_images,skip)
    show_camvid_ssfpn_model(num_images,skip)
    show_camvid_bisenet_model(num_images,skip)
