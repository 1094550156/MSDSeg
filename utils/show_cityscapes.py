...
def display_cityscapes():
    ...
    random.seed(seed)
    np.random.seed(seed)
    config_filename= "configs/cityscapes_500epochs_zdence.yaml"
...
def show_cityscapes_model(num_images,skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "configs/cityscapes_1000epochs_zdence.yaml"
    with open(config_filename) as file:
        config=yaml.full_load(file)
    from zdence_mult_xiaorong.Zdence_mult_pq310 import ZNet_v2
    model = ZNet_v2(
        num_classes=19,
        pretrained = "checkpoints/1000epochs_Zdence_mult_pq310_run1"
    )
...
def show_cityscapes_ohem_model(num_images,skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "configs/cityscapes_1000epochs_zdence.yaml"
    with open(config_filename) as file:
        config=yaml.full_load(file)
    from zdence_mult_xiaorong.Zdence_mult_pq312 import ZNet_v2
    model = ZNet_v2(
        num_classes=19,
        pretrained="checkpoints/1000epochs_Zdence_mult_pq312_run1",
    )
    num_images=num_images
    images_per_line=1
    skip=skip
...
def show_cityscapes_ohem_model1(num_images,skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "configs/cityscapes_1000epochs_zdence.yaml"
    with open(config_filename) as file:
        config=yaml.full_load(file)
    from zdence_mult_xiaorong.Zdence_mult_pq314 import ZNet_v2
    model = ZNet_v2(
        num_classes=19,
        pretrained="checkpoints/1000epochs_Zdence_mult_pq314_run1",
    )
    num_images=num_images
    images_per_line=1
    skip=skip
...
def show_cityscapes_bisenet_model(num_images,skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "configs/cityscapes_1000epochs_zdence.yaml"
    with open(config_filename) as file:
    ...
    from zdence_mult_xiaorong.Zdence_mult_pq610 import ZNet_v2
    # model = BiSeNetV1(
    #     n_classes=19,
    #     pretrained = "checkpoints/bisenet_res18_run1"
    # )
    model = ZNet_v2(
        num_classes=19,
        pretrained="checkpoints/1000epochs_Zdence_mult_pq610_run1",
    )
    num_images=num_images
    images_per_line=1
    skip=skip
...
def show_cityscapes_erfnet_model(num_images, skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "configs/cityscapes_1000epochs_zdence.yaml"
    with open(config_filename) as file:
        config = yaml.full_load(file)
    # from competitors_models.erfnet import ERFNet
    from zdence_mult_xiaorong.Zdence_mult_baseline_s1 import ZNet_v2
    # model = ERFNet(
    #     classes=19,
    #     pretrained="checkpoints/200epochs_erfnet_9.4_run1"
    # )
    model = ZNet_v2(
        num_classes=19,
        pretrained="checkpoints/1000epochs_Zdence_mult_baseline_s1_run1",
    )
    num_images = num_images
    images_per_line = 1
    skip = skip
    ...
    np.random.seed(seed)
    train_loader, val_loader, train_set = get_dataset_loaders(config)
    show(model,val_loader, device, show_cityscapes_mask, num_images=num_images, skip=skip,
         images_per_line=images_per_line)


def show_cityscapes_fbsnet_model(num_images,skip):
    import random
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    config_filename = "configs/cityscapes_1000epochs_zdence.yaml"
    with open(config_filename) as file:
        config = yaml.full_load(file)
    # from competitors_models.fbsnet import FBSNet
    from zdence_mult_xiaorong.Zdence_mult_pq614 import ZNet_v2
    # model = FBSNet(
    #     classes=19,
    #     pretrained="checkpoints/1000epochs_fbsnet_9.1_latest"
    # )
    model = ZNet_v2(
        num_classes=19,
        pretrained="checkpoints/1000epochs_Zdence_mult_pq614_73.34_run1",
    )
    num_images = num_images
    images_per_line = 1
    skip = skip
...
if __name__=="__main__":
    ...
    show_cityscapes_ohem_model(num_images, skip)
    show_cityscapes_ohem_model1(num_images, skip)
    show_cityscapes_bisenet_model(num_images, skip)
    show_cityscapes_erfnet_model(num_images, skip)
    show_cityscapes_fbsnet_model(num_images, skip)
